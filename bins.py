...
z = np.array([2.2, 2.3, 2.4, 2.5, 2.6, 2.7, 2.8, 2.9, 3.0, 3.5, 4.0, 4.5, 5.5, 7.0])
...
for i, rs in enumerate(z[:-1]):
    ...
    lfi.run_mcmc()
    lfi.get_percentiles()
    lfi.draw(lfi.z.mean(), dirname='set7/', plotlit=True)
...
if write_percentiles:
    ...
    np.savez('percs_full.npz', zs=zs, phi_star=phi_star, m_star=m_star,
