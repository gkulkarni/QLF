...
def j(emodel, loglf=None, theta=None, dz=0.1, n_ws=200, n_ws_int=100, zmax=7.0):
    ...
    for z in zs:
        ...
        # [je] = erg s^-1 Hz^-1 cm^-2 sr^-1
        je = je + (e*c_mpcPerYr*np.abs(dtdz(z))*dz*cmbympc**2)/(4.0*np.pi) 

        nu_rest = c_angPerSec*(1.0+z)/ws 
        t = np.array([tau_eff(x, z) for x in nu_rest])
        je = je*np.exp(-t*dz)

        n = 4.0*np.pi*je*(1.0+z)**3/(hplanck*nu_rest)
...
def draw_g_paper():
    ...
    if tabulate:
        ...
        e912_18 = e1450_18*((912.0/1450.0)**0.61)
        e912_18_up = e1450_18_up*((912.0/1450.0)**0.61) 
        e912_18_low = e1450_18_low*((912.0/1450.0)**0.61)

        e912_21 = e1450_21*((912.0/1450.0)**0.61)
        e912_21_up = e1450_21_up*((912.0/1450.0)**0.61)
        e912_21_low = e1450_21_low*((912.0/1450.0)**0.61)
...
def draw_g_referee(zmax):
    ...
    plt.text(5.4, 1.0, legend_title) 

    outfile = 'g_zmax{:g}.pdf'.format(zmax)
...
# draw_g_referee(zmax=15)
draw_g_referee(zmax=10)
draw_g_referee(zmax=7)
draw_g_referee(zmax=6.5)
draw_g_referee(zmax=6)
draw_g_referee(zmax=5)
draw_g_referee(zmax=3)
