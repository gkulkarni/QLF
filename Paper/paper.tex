...
In a magnitude bin $[M_\mathrm{min}, M_\mathrm{max})$, and redshift
  ...
  points in Figures \ref{fig:mosaic} and \ref{fig:mosaic2}.  These
  agree quite well with the published luminosity functions by these
  surveys.  
...
function parameters, and $\{M_i, z_i\}$ denotes the complete quasar
data.  We use a uniform prior distribution $p(\phi_*, M_*, \alpha,
\beta)$ and assume that the likelihood
\begin{equation}
  \mathcal{L}\equiv p(\{M_i, z_i\} | \phi_*, M_*, \alpha, \beta)
\end{equation}
is given by $\phi(M)$ with suitable normalisation.  That is the
negative logarithm of the likelihood $S\equiv -2\ln\mathcal{L}$ can be
written as
\begin{multline}
...
integral in the second term on the right hand side is on the 
surveyed range of $M$ and $z$.

Another way of understanding the above likelihood is as limit of the
...
infinitesimal bins, $n_{ij}=0$ or $1$, in which case
Equation~(\ref{eqn:lhood}) can be simplified and we can get
Equation~(\ref{eqn:S}).

Our estimates for the luminosity function in several redshift bins are
shown in Figures \ref{fig:mosaic} and \ref{fig:mosaic2}.  The median
of the posterior is shown as the best fit model.  Yellow curves show a
random draw from the posterior.

\subsection{Luminosity function estimate without binning}

It is useful to go beyond the redshift bins of the previous section.
This gives us continuous models of luminosity function evolution and
is also relevant for extrapolation. We therefore obtain estimates of
...
\begin{figure*}
  \begin{center}
    ...
    \includegraphics[height=0.9\textheight,keepaspectratio]{mosaic_small.pdf}
  \end{center}
