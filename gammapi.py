...
def fnu(nu, loglf, theta, M, z, fit='composite'):
    # SED power law index is from Beta's paper.
    L = luminosity(M)
    w = c_angPerSec / nu
    ...
    return (10.0**loglf(theta, M, z))*e

vfnu = np.vectorize(fnu, otypes=[np.float])
    
def e912(loglf, theta, z, mlims, fit='composite'):
...
def emissivity(w, z, composite=loglf, lfparams=theta):

    mlims = (-30.0, -20.0)
    m = np.linspace(mlims[0], mlims[1], num=1000)
    nu = c_angPerSec/w
    farr = vfnu(nu, loglf, theta, m, z)
    return np.trapz(farr, m) # erg s^-1 Hz^-1 Mpc^-3
    
def get_emissivity(lfi, z):

    rindices = np.random.randint(len(lfi.samples), size=300)
    e = np.array([e912(lfi.log10phi, theta, z, (-30.0, -20.0), fit='individual')
                          for theta
...
def Gamma_HI(loglf, theta, z, fit='composite'):
    ...
    em = e912(loglf, theta, z, (-30.0, -23.0), fit=fit_type)
    alpha_EUV = -1.7
    part1 = 4.6e-13 * (em/1.0e24) * ((1.0+z)/5.0)**(-2.4) / (1.5-alpha_EUV) # s^-1 

    em = e912(loglf, theta, z, (-23.0, -20.0), fit=fit_type)
    alpha_EUV = -0.56
...
def Gamma_HI_singleslope(loglf, theta, z, fit='composite'):
    ...
    # Taken from Equation 11 of Lusso et al. 2015.
    em = e912(loglf, theta, z, (-30.0, -20.0), fit=fit_type)
    alpha_EUV = -1.7
...
def draw_emissivity(all_individuals, zlims, composite=None, select=False):
    ...
    if composite is not None:
        ...
        e = [e912(composite.log10phi, bf, x, (-30.0, -20.0)) for x in zc]
        for theta in composite.samples[np.random.randint(len(composite.samples),
                                                         size=300)]:
            e = [e912(composite.log10phi, theta, x, (-30.0, -20.0)) for x in zc]
